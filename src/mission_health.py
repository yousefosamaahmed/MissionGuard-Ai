...
def calculate_mission_health_score(
    ...
    if missing_columns:
        raise ValueError("Mission-health input is missing columns: " + ", ".join(missing_columns))
    ...
    telemetry_stability = float(np.clip(100.0 - mean_risk, 0.0, 100.0))
    anomaly_control = float(np.clip(100.0 - anomaly_rate, 0.0, 100.0))
    peak_resilience = float(np.clip(100.0 - peak_risk, 0.0, 100.0))

    incident_burden = 0.0
    unresolved_incidents = 0

    for incident in incidents:
        severity = str(incident.get("severity", "watch")).strip().lower()
        status = str(incident.get("status", "open")).strip().lower()
        ...
        incident_burden += SEVERITY_WEIGHTS.get(severity, 0.35) * status_multiplier
    ...
    if incidents:
        ...
        incident_readiness = float(100.0 * (1.0 - normalized_burden))
    else:
        incident_readiness = 100.0

    score = float(
        np.clip(
            telemetry_stability * MISSION_HEALTH_WEIGHTS["telemetry_stability"]
            + anomaly_control * MISSION_HEALTH_WEIGHTS["anomaly_control"]
            + peak_resilience * MISSION_HEALTH_WEIGHTS["peak_resilience"]
            + incident_readiness * MISSION_HEALTH_WEIGHTS["incident_readiness"],
            0.0,
            100.0,
        )
    )
