...
def create_telemetry_session(
    ...
    if not clean_session_name:
        raise ValueError("Telemetry session name cannot be empty.")

    if mission_id is None and dataset_id is None:
        raise ValueError("A telemetry session must reference a mission, a dataset, or both.")

    if source_type not in VALID_SESSION_SOURCE_TYPES:
        raise ValueError(f"Invalid session source type: {source_type}")
    ...
    sessions_table = get_table("telemetry_sessions")
    ...
    with database_session() as session:
        telemetry_session_id = session.execute(statement).scalar_one()
...
def get_telemetry_session(
    ...
    statement = select(sessions_table).where(sessions_table.c.id == telemetry_session_id)
...
def list_telemetry_sessions(
    ...
    statement = select(sessions_table)

    if mission_id is not None:
        statement = statement.where(sessions_table.c.mission_id == mission_id)

    if dataset_id is not None:
        statement = statement.where(sessions_table.c.dataset_id == dataset_id)

    statement = statement.order_by(sessions_table.c.created_at.desc())
