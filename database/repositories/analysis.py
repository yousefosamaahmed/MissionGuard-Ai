...
def create_analysis_run(
    ...
    if run_type not in VALID_RUN_TYPES:
        raise ValueError(f"Invalid analysis run type: {run_type}")

    if status not in VALID_RUN_STATUSES:
        raise ValueError(f"Invalid analysis run status: {status}")

    analysis_runs = get_table("analysis_runs")
    ...
    statement = (
        ...
        .returning(analysis_runs.c.id)
    )

    with database_session() as session:
        analysis_run_id = session.execute(statement).scalar_one()

    return _to_uuid(analysis_run_id)
...
def save_prediction_batch(
    ...
    predictions = get_table("predictions")
    ...
    for record in prediction_records:
        telemetry_sample_id = int(record["telemetry_sample_id"])

        if telemetry_sample_id in seen_sample_ids:
            raise ValueError(
                f"Duplicate telemetry_sample_id inside prediction batch: {telemetry_sample_id}"
            )

        seen_sample_ids.add(telemetry_sample_id)
        ...
        optional_scores = {
            ...
            "forecast_residual_score": record.get("forecast_residual_score"),
            "rule_score": record.get("rule_score"),
            "persistence_score": record.get("persistence_score"),
            "early_warning_score": record.get("early_warning_score"),
        }

        for score_name, score_value in optional_scores.items():
            if score_value is not None:
                optional_scores[score_name] = float(score_value)
        ...
        if not isinstance(
            ...
            raise TypeError("affected_subsystems must be a list.")

        if not isinstance(
            rule_violations,
            list,
        ):
            raise TypeError("rule_violations must be a list.")

        prediction_rows.append(
            {
                "analysis_run_id": analysis_run_id,
                "telemetry_sample_id": (telemetry_sample_id),
                "predicted_anomaly": bool(record["predicted_anomaly"]),
                "risk_level": risk_level,
                "risk_score": risk_score,
                "confidence_score": (confidence_score),
                "isolation_score": (optional_scores["isolation_score"]),
                "forecast_residual_score": (optional_scores["forecast_residual_score"]),
                "rule_score": (optional_scores["rule_score"]),
                "persistence_score": (optional_scores["persistence_score"]),
                "early_warning_score": (optional_scores["early_warning_score"]),
                "top_feature": record.get("top_feature"),
    ...
    insert_statement = pg_insert(predictions).values(prediction_rows)

    statement = insert_statement.on_conflict_do_update(
        constraint=("uq_analysis_sample_prediction"),
        set_={
            "predicted_anomaly": (insert_statement.excluded.predicted_anomaly),
            "risk_level": (insert_statement.excluded.risk_level),
            "risk_score": (insert_statement.excluded.risk_score),
            "confidence_score": (insert_statement.excluded.confidence_score),
            "isolation_score": (insert_statement.excluded.isolation_score),
            "forecast_residual_score": (insert_statement.excluded.forecast_residual_score),
            "rule_score": (insert_statement.excluded.rule_score),
            "persistence_score": (insert_statement.excluded.persistence_score),
            "early_warning_score": (insert_statement.excluded.early_warning_score),
            "top_feature": (insert_statement.excluded.top_feature),
            "out_of_distribution": (insert_statement.excluded.out_of_distribution),
            "human_review_required": (insert_statement.excluded.human_review_required),
            "explanation": (insert_statement.excluded.explanation),
            "feature_contributions": (insert_statement.excluded.feature_contributions),
            "rule_violations": (insert_statement.excluded.rule_violations),
            "affected_subsystems": (insert_statement.excluded.affected_subsystems),
            "prediction_metadata": (insert_statement.excluded.prediction_metadata),
        },
    ).returning(
        predictions.c.id,
    ...
    with database_session() as session:
        stored_rows = session.execute(statement).mappings().all()

    if len(stored_rows) != len(prediction_records):
        raise RuntimeError("Stored prediction count does not match the submitted prediction count.")
    ...
    for stored_row in stored_rows:
        telemetry_sample_id = int(stored_row["telemetry_sample_id"])

        prediction_id = int(stored_row["id"])

        prediction_ids_by_sample[telemetry_sample_id] = prediction_id

        if bool(stored_row["predicted_anomaly"]):
            anomaly_predictions += 1

    return PredictionSaveResult(
        stored_predictions=len(stored_rows),
        anomaly_predictions=anomaly_predictions,
        prediction_ids_by_sample=(prediction_ids_by_sample),
    )
...
def create_incident(
    ...
    if not clean_incident_code:
        raise ValueError("Incident code cannot be empty.")
    ...
    if duration_samples is not None and duration_samples < 1:
        raise ValueError("duration_samples must be at least 1.")

    incidents = get_table("incidents")

    insert_statement = pg_insert(incidents).values(
        analysis_run_id=analysis_run_id,
    ...
    statement = insert_statement.on_conflict_do_update(
        ...
        set_={
            "start_sample_id": (insert_statement.excluded.start_sample_id),
            "end_sample_id": (insert_statement.excluded.end_sample_id),
            "started_at": (insert_statement.excluded.started_at),
            "ended_at": (insert_statement.excluded.ended_at),
            "duration_samples": (insert_statement.excluded.duration_samples),
            "severity": (insert_statement.excluded.severity),
            "peak_risk_score": (insert_statement.excluded.peak_risk_score),
            "peak_confidence": (insert_statement.excluded.peak_confidence),
            "top_feature": (insert_statement.excluded.top_feature),
            "affected_subsystems": (insert_statement.excluded.affected_subsystems),
            "human_review_required": (insert_statement.excluded.human_review_required),
...
def complete_analysis_run(
    ...
    if total_anomalies < 0:
        raise ValueError("total_anomalies cannot be negative.")

    if total_incidents < 0:
        raise ValueError("total_incidents cannot be negative.")

    if total_anomalies > total_predictions:
        raise ValueError("total_anomalies cannot exceed total_predictions.")
    ...
    if updated_id is None:
        raise RuntimeError(f"Analysis run was not found: {analysis_run_id}")
...
def get_analysis_run(
    ...
    statement = select(analysis_runs).where(analysis_runs.c.id == analysis_run_id)
...
def list_predictions(
    ...
    statement = (
        select(predictions)
        .where(predictions.c.analysis_run_id == analysis_run_id)
        .order_by(predictions.c.telemetry_sample_id.asc())
    )
...
def list_incidents(
    ...
    statement = (
        select(incidents)
        .where(incidents.c.analysis_run_id == analysis_run_id)
...
def get_latest_completed_analysis_run(
    ...
    statement = (
        ...
        .where(analysis_runs.c.model_version_id == model_version_id)
        .where(analysis_runs.c.status == "completed")
        .order_by(
            analysis_runs.c.finished_at.desc().nullslast(),
...
def get_prediction_risk_summary(
    ...
    anomaly_count_expression = func.sum(
        case(
            (
                predictions.c.predicted_anomaly.is_(True),
                1,
            ),
            else_=0,
        )
    ...
    statement = select(
        func.count(predictions.c.id).label("total_predictions"),
        func.coalesce(
            anomaly_count_expression,
            0,
        ).label("total_anomalies"),
        func.coalesce(
            func.avg(predictions.c.risk_score),
            0.0,
        ).label("mean_risk_score"),
        func.coalesce(
            func.max(predictions.c.risk_score),
            0.0,
...
def list_analysis_prediction_records(
    ...
    telemetry_samples = get_table("telemetry_samples")

    feature_vectors = get_table("feature_vectors")
    ...
    statement = (
        ...
        .join(
            telemetry_samples,
            telemetry_samples.c.id == predictions.c.telemetry_sample_id,
        )
        .join(
            feature_vectors,
            feature_vectors.c.telemetry_sample_id == telemetry_samples.c.id,
        )
        .where(predictions.c.analysis_run_id == analysis_run_id)
    )

    if schema_name is not None:
        statement = statement.where(feature_vectors.c.schema_name == schema_name)

    statement = statement.order_by(telemetry_samples.c.sample_index.asc())
...
def update_incident_review(
    ...
    with database_session() as session:
        ...
        existing_incident = session.execute(existing_statement).mappings().one_or_none()

        if existing_incident is None:
            raise RuntimeError(f"Incident was not found: {incident_id}")

        previous_status = str(existing_incident["status"])

        existing_metadata = existing_incident["metadata"]

        if isinstance(
            existing_metadata,
            dict,
        ):
            incident_metadata = dict(existing_metadata)
        else:
            incident_metadata = {}

        existing_history = incident_metadata.get(
            "review_history",
            [],
        )
        ...
        if isinstance(
            ...
            review_history = list(existing_history)
        else:
            review_history = []
        ...
        updated_incident = session.execute(update_statement).mappings().one()
...
def update_analysis_run_mission_health(
    ...
    with database_session() as session:
        ...
        existing_run = session.execute(existing_statement).mappings().one_or_none()
        ...
        merged_metadata["mission_health"] = dict(health_snapshot)

        update_statement = (
            update(analysis_runs)
            .where(analysis_runs.c.id == analysis_run_id)
            .values(
                mission_health_score=float(mission_health_score),
                metadata=merged_metadata,
            )
            .returning(*analysis_runs.c)
        )

        updated_run = session.execute(update_statement).mappings().one()
