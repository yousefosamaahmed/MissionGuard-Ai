...
def find_model_artifact() -> Path | None:
    ...
    for file_path in sorted(models_directory.rglob("*")):
...
def main() -> None:
    ...
    training_dataset_id = find_latest_opssat_dataset_id()

    print(f"Training dataset ID: {training_dataset_id}")

    model_artifact = find_model_artifact()

    model_size_bytes = model_artifact.stat().st_size if model_artifact is not None else None

    version = "local-" + uuid4().hex[:8].lower()

    print("\n2. Registering model version...")

    model_version_id = create_model_version(
        model_name=("MissionGuard OPS-SAT Anomaly Detector"),
        model_type="isolation_forest",
        version=version,
        description=("OPS-SAT segment-level anomaly detection model."),
    ...
    if model_artifact is not None:
        ...
        artifact_id = register_model_artifact(
            ...
            artifact_type=detect_artifact_type(model_artifact),
            file_path=model_artifact,
            storage_provider="local",
            metadata={
                "original_file_name": (model_artifact.name),
            },
        )

        print(f"Artifact file: {model_artifact}")

        print(f"Artifact ID: {artifact_id}")

    else:
        print("\n3. No .joblib, .pkl, .onnx, .pt or .pth model file was found.")

        print("The model version was registered without an artifact.")

    print("\n4. Reading stored model...")

    stored_model = get_model_version(model_version_id)

    if stored_model is None:
        raise RuntimeError("The registered model could not be found.")

    print(f"Model name: {stored_model['model_name']}")

    print(f"Model type: {stored_model['model_type']}")

    print(f"Version: {stored_model['version']}")

    print(f"Status: {stored_model['status']}")

    artifacts = list_model_artifacts(model_version_id)
